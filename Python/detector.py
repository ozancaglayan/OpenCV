...
OPENCV_PATH = "/usr/share/OpenCV/haarcascades"
...
class CascadedDetector(Detector):
    ...
    works*. 
    """
    def __init__(self, cascade_fn=os.path.join(OPENCV_PATH, "haarcascade_frontalface_alt2.xml"), scaleFactor=1.2, minNeighbors=5, minSize=(30,30)):
...
class SkinFaceDetector(Detector):
    ...
    """
    def __init__(self, threshold=0.3, cascade_fn=os.path.join(OPENCV_PATH, "haarcascade_frontalface_alt2.xml"),
...
if __name__ == "__main__":
    ...
    detector = CascadedDetector(cascade_fn=os.path.join(OPENCV_PATH,"haarcascade_frontalface_alt2.xml"))

    # detection
    for i,r in enumerate(detector.detect(img)):
        x0,y0,x1,y1 = r
        print "Found face #%d between (%d,%d) and (%d,%d)" % (i, x0, y0, x1, y1)
